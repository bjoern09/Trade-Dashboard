...
- Umsatzwachstum-Kontext ueber KGV/EV-Kennzahlen: EV/EBITDA ${latestMetrics.enterpriseValueOverEBITDA ?? "n/v"}
...
${news.length > 0 ? news.map((n) => `- ${n.title} (${n.source}, ${n.date})`).join("\n") : "Keine Nachrichtendaten verfuegbar - stuetze die Einordnung ausschliesslich auf Kursverlauf und Fundamentaldaten."}
...
Erstelle die edukative Einordnung gemaess Systemvorgabe.`;
  ...
  if (!textBlock) {
    throw new Error(
      `Claude hat keine Textantwort geliefert (stop_reason: ${data.stop_reason ?? "unbekannt"}). ` +
      `Moeglicherweise war max_tokens zu knapp bemessen.`
    );
  }
